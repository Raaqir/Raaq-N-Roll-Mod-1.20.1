...
public class MetalDetectorItem extends Item {
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        if(!context.getWorld().isClient()) {
            ...
            if(!foundBlock) {
                player.sendMessage(Text.translatable("item.raaqnroll.metal_detector.no_valuables"));
            }
        }
    ...
    public void appendTooltip(ItemStack stack, @Nullable World world, List<Text> tooltip, TooltipContext context) {
        if(Screen.hasShiftDown()) {
            tooltip.add(Text.translatable("tooltip.raaqnroll.metal_detector.tooltip.shift"));
        } else {
            tooltip.add(Text.translatable("tooltip.raaqnroll.metal_detector.tooltip"));
        }
    }
